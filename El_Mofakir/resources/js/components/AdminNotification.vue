 <template>
     <li class="nav-item dropdown no-arrow mx-1">
         ...
         <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in"
              aria-labelledby="alertsDropdown" v-if="unreadCount > 0">
             ...
             <a class="dropdown-item d-flex align-items-center" v-for="item in unread" :key="item.id" :href="`/admin/post_comments/${item.data.id}/edit`" @click="readNotifications(item)">
                 ...
                 <div>
                     ...
                     <span class="font-weight-bold">There is a new comment on post: {{ item.data.post_title }}</span>
                 </div>
             </a>
    ...
    methods: {
        getNotifications() {
            axios.get('/admin/notifications/get').then(res => {
                ...
                console.error(error);
                // Or use any custom error handling logic here
            });
        },
