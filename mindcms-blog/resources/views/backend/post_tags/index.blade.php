        ...
        @include('backend.post_tags.filter.filter')
            <div class="table-responsive">
                <table class="table table-hover">
                    ...
                    @forelse($tags as $tag)
                        <tr>
                            <td>{{ config('app.locale') == 'ar' ? $tag->name : $tag->name_en }}</td>
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <a href="javascript:void(0);" onclick="if(confirm('{{__('Backend/post_tags.are_you_sure')}} ')) { document.getElementById('tag-delete-{{$tag->id}}').submit(); } else { return false}" class="btn btn-danger"><i class="fa fa-trash"></i></a>
